/****** Object:  StoredProcedure [dbo].[SP_UNIDADES_CON_EFIC_ENERG_TODOS_LOS_SERVICIOS]    Script Date: 26/06/2020 15:17:00 ******/
SET ANSI_NULLS ON
GO
...
/*
-- =============================================
-- Author:		Ivan Molina H (APIUX) - sigmavirus@gmail.com
-- Create date: 26/06/2020
-- Description:	Base preliminar de datos para reporte de Unidades por Servicio Con Eficiencia Energetica
                para Excel con [ServicioId].xlsx. Se filtrara por unidades con campo ReportaPMG = 1
				(tabla Divisiones) y que las tablas con que se crucen tengan Active = 1
--              SUPUESTO: No necesita campo ficticio Int64PK pues no se muestra en Excel, la primera
                columna es DivisionId, y el order: ORDER BY Servicio.Id, Region.Posicion, Division.Id
				Luego para ordenar por "Nombre de Comuna" eso se hará en la APLICACIÓN ORQUESTADORA
				para no agregar carga de trabajo innecesaria a la BBDD
-- =============================================
 EXEC [SP_UNIDADES_CON_EFIC_ENERG_TODOS_LOS_SERVICIOS] */
CREATE PROCEDURE [dbo].[SP_UNIDADES_CON_EFIC_ENERG_TODOS_LOS_SERVICIOS]
AS
BEGIN
...
insert @anexo_servicio (ServicioId, AnexoId) values(551, 	142)
	...
	declare @listado_ids_servicio table
	(ServicioId bigint PRIMARY KEY,
	NombreServicio nvarchar(1024) NULL)

	declare @listado_ids_divisiones_efic_energ table
	(UnidadId bigint PRIMARY KEY)

	insert @listado_ids_divisiones_efic_energ
	select d.Id from Divisiones d
	where d.ReportaPMG = 1 and d.Active = 1

	insert @listado_ids_servicio
	select s.Id, s.Nombre from Servicios s
	where s.Active = 1
	order by s.Id

	--select * from @medidores_por_division mdd
	--order by mdd.DivisionId

	select
	d.Id 'IdUnidad'
	, s.NombreServicio 'NombreServicio'
	, s.ServicioId 'ServicioId'
	, ISNULL(a.AnexoId,0) 'AnexoId'
	, co.RegionId 'RegionId'
	, r.Posicion 'RegionPos'
	, co.Nombre 'NombreComuna'
	, d.Direccion 'NombreUnidad'
	from Divisiones d
	inner join @listado_ids_divisiones_efic_energ de on de.UnidadId = d.Id
	inner join @listado_ids_servicio s on s.ServicioId = d.ServicioId
	inner join Edificios e on e.Id = d.EdificioId
	inner join Comunas co on co.Id = e.ComunaId
	inner join Regiones r on r.Id = co.RegionId
	left join @anexo_servicio a on a.ServicioId = s.ServicioId
--	and d.ServicioId = @ServicioId
	order by s.ServicioId, r.Posicion, d.Id
END